...
// SetEditAllowlist restricts mutating path-based tools to the given workspace paths.
// Empty paths disable the allowlist restriction.
func (r *Registry) SetEditAllowlist(workspaceRoot string, paths []string, source string) {
	r.mu.Lock()
	defer r.mu.Unlock()

	r.workspaceRoot = filepath.Clean(workspaceRoot)
	r.editAllowlist = normalizeAllowlistPaths(paths)
	r.editAllowlistSource = strings.TrimSpace(source)
}
...
func (r *Registry) Execute(ctx context.Context, name string, args json.RawMessage) (*Result, error) {
	...
	if blockMsg := r.mutationPathAllowlistBlock(tool, args); blockMsg != "" {
...
func (r *Registry) mutationPathAllowlistBlock(tool Tool, args json.RawMessage) string {
	if !tool.IsMutating() {
		return ""
	}
	...
	allowlist := append([]string(nil), r.editAllowlist...)
	source := r.editAllowlistSource
	r.mu.RUnlock()
	...
	if err := json.Unmarshal(args, &parsed); err != nil || strings.TrimSpace(parsed.Path) == "" {
		return fmt.Sprintf("Tool %q blocked: mutating tool call must include a path when edit allowlist is active.", tool.Name())
	}
	if pathAllowedByAllowlist(workspaceRoot, parsed.Path, allowlist) {
		return ""
	}
	if source == "" {
	...
	return fmt.Sprintf(
		"Tool %q blocked by %s: path %q is outside the allowed edit paths: %s. "+
			"Do not edit this file; report the needed change and ask for explicit permission.",
		tool.Name(), source, parsed.Path, strings.Join(allowlist, ", "),
	)
}

func normalizeAllowlistPaths(paths []string) []string {
...
func pathAllowedByAllowlist(workspaceRoot string, path string, allowlist []string) bool {
	rel := normalizeMutatingPathForAllowlist(workspaceRoot, path)
	if rel == "" {
		return false
	}
	for _, allowed := range allowlist {
		if strings.HasSuffix(allowed, "/") {
			...
			if rel == prefix || strings.HasPrefix(rel, allowed) {
				return true
			}
			continue
		}
...
func normalizeMutatingPathForAllowlist(workspaceRoot string, path string) string {
	path = strings.TrimSpace(path)
	if path == "" {
		return ""
	}
	...
	if filepath.IsAbs(path) {
		...
		return normalizeAllowlistPath(path)
	}
