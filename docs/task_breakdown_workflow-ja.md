...
```text
 ...
 - 編集ステップは、各編集の挿入行数が40行未満で完了できる程度に十分に小さくすること。
...
VMAXの巨大編集安全機能（large-edit safety）によって編集が拒否された場合、その拒否を「さらなるタスク分割のシグナル」として捉えてください。処理を停止したり同じペイロードを再試行したりするのではなく、より小さく調整した新規の編集で作業を続行してください。
